...
def get_cb_loss(config):
    ...
    for i in range(len(config.class_weights)):
        ...
        loss_fns.append( CBLoss(class_cnt, len(class_cnt), loss_type, beta, gamma) )

    if config.use_normsoftmax:
        from core.model.loss_module.normsoftmax import NormSoftmaxLoss
        dim = 512
        
        for i in range(len(config.class_weights)):
            class_cnt = len(config.class_cnt[i])
